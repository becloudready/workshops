variable "student_name" {
  description = "Your name in lowercase with no spaces (e.g. amiel-halili). Used to identify your resources."
  default     = "amiel-halili"

  validation {
    condition     = can(regex("^[a-z0-9-]+$", var.student_name))
    error_message = "student_name must be lowercase letters, numbers, and hyphens only (e.g. amiel-halili)."
  }
}
...
variable "created_date" {
  description = "Creation date for the `date` tag, format dd-mmm-yyyy (e.g. 31-Jul-2026)."
  type        = string
}
