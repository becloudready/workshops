...
from data.models import Account, Transaction, Transfer, User
...
ACCOUNTS = [
    {"owner_email": "alice@example.com", "account_type": "checking", "balance": Decimal("2500.00")},
    {"owner_email": "alice@example.com", "account_type": "savings", "balance": Decimal("11000.50")},
    {"owner_email": "bob@example.com", "account_type": "checking", "balance": Decimal("420.75")},
]
...
def seed(session) -> None:
    ...
    for spec in ACCOUNTS:
        account = Account(
            user_id=users_by_email[spec["owner_email"]].id,
    ...
    # Mirrors routers/transfers.py: a transfer produces one Transfer record
    # plus a send/receive Transaction pair on the two accounts involved.
    transfers: list[Transfer] = []
    for spec in TRANSACTIONS:
        from_account = accounts[spec["from_index"]]
        to_account = accounts[spec["to_index"]]
    ...
    session.commit()
    print(f"Seeded {len(USERS)} users, {len(accounts)} accounts, {len(transfers)} transfers.")
